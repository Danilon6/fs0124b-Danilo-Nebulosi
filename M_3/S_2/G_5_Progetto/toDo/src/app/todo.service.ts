...
type userWithToDOsObject = {
  [key: number]: IUserWithToDos
}
...
export class TodoService {
  ...
  constructor(private userSvc: UsersService) {
    ...
    this.userWithToDOs.next(this.userWithToDos)
  ...
  userWithToDOs = new BehaviorSubject<IUserWithToDos[]>([])

  $userWithToDOs = this.userWithToDOs.asObservable()

  combinedToDoArrAndUserArr(todoArr: iTodo[], userArr: iUsers[]) {
  ...
  CombineUserWithToDos(todoArr: iTodo[], userArr: iUsers[]) {

    let combinedObject = userArr.reduce((acc: userWithToDOsObject, user) => {
      const todos = todoArr.filter(todo => todo.userId === user.id);
